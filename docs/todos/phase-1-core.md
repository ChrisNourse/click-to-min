...
- **Acceptance criteria**:
  - [ ] Protocol compiles inside `ClickToMinCore`
  - [ ] No AppKit/AX imports
  - [ ] Symbol visible to Phase 2 I/O target (confirm via a throwaway conformance in I/O)
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] Accepts injected `[CGRect]` — no `NSScreen.screens` read in Core
  - [ ] Handles single screen (bottom-left → top-left)
  - [ ] Handles multi-screen (Dock on secondary left/right/above/below primary)
  - [ ] Handles negative-origin screens (PLAN.md §Testing Layer 1)
  - [ ] Retina: returns points, not pixels
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] `contains` returns true strictly inside, true on edge, false outside
  - [ ] Nil frame from provider → returns false
  - [ ] Test covers Dock at bottom / left / right
  - [ ] Test covers frame change between two calls (injected provider mutates)
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] `static let debounceInterval: TimeInterval = 0.3` exists
  - [ ] Same item within 300ms → suppressed
  - [ ] Same item after 300ms → allowed
  - [ ] Different item within 300ms → allowed
  - [ ] Injected clock used (no `Date()` in impl)
  - [ ] Key is the bundle URL string (test renames/relaunches reuse key)
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] Trailing slash normalization: `/A/Safari.app/` == `/A/Safari.app`
  - [ ] Symlink resolution: symlinked path == real path
  - [ ] `file://` form normalized
  - [ ] Nil on either side → false (never crash)
  - [ ] Case-sensitive: `/A/safari.app` != `/A/Safari.app`
  - [ ] System-volume path behavior locked with an explicit test + comment
- **Verification step**:
...
- [ ] Any Core file imports AppKit, AX, or Foundation/NSWorkspace — compile-time forbidden by target split, but verify visually
- [ ] `Date()` or `CFAbsoluteTime` used directly in debouncer — must use injected clock
- [ ] `NSScreen.main` read inside `CoordinateConverter` — must be injected
- [ ] Test uses real wall-clock `Thread.sleep` for debounce timing — must use the injected clock closure
- [ ] Case-folding snuck into URL matcher (`lowercased()` anywhere) — explicit anti-goal
