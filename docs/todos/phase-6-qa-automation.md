...
- **Acceptance criteria**:
  - [ ] `qa/lib/common.sh` sources cleanly under `bash -n`
  - [ ] Helpers are unit-smokeable by calling each in isolation (e.g. `qa::build_release` produces `ClickToMin.app`)
  - [ ] `qa/README.md` documents prerequisites: Xcode CLT, `cliclick` (brew), Accessibility grant for both `ClickToMin.app` and the `AXProbe` harness
  - [ ] `qa/reports/` is gitignored but the directory is kept via `.gitkeep`
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] Builds via `swift build --package-path qa/harness/AXProbe`
  - [ ] Exits non-zero if Accessibility is not granted; error message explains grant path
  - [ ] All 4 subcommands covered by a `qa/harness/AXProbe/Tests` test target with in-memory fakes
  - [ ] Documented requirement: user grants Accessibility to the compiled `axprobe` binary once (separate from `ClickToMin.app`)
- **Verification step**:
...
- **Acceptance criteria**:
  ...
  - [ ] `./build.sh` produces `ClickToMin.app`
  - [ ] `codesign --verify --verbose ClickToMin.app` passes
  - [ ] `plutil -lint Resources/Info.plist` passes
  - [ ] Launches `.app`, captures 10s of `log stream --predicate 'subsystem == "com.click-to-min"'`, asserts all three `lifecycle` signposts fire: *permission granted* (or *awaiting permission* if not pre-granted), *monitor installed*, *Dock PID refreshed*
  - [ ] Quits cleanly via `osascript -e 'tell application "ClickToMin" to quit'`; asserts no stray process via `pgrep -x ClickToMin`
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] Case 1: background app → Dock click → `frontmost-bundle-id` becomes target (no minimize)
  - [ ] Case 2: target frontmost → Dock click → `axprobe wait-until-minimized target --timeout 0.5` succeeds
  - [ ] Case 3: after Case 2, Dock click → `is-minimized` becomes `false` (macOS restore, no interference)
  - [ ] Full 3-click cycle completes in under 3s wall-clock
  - [ ] Click→minimize latency p95 across 20 trials recorded and compared to `PERF.md` threshold (default <50ms)
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] Frozen-app timeout: launch target, `kill -STOP <pid>`, click Dock icon, pipeline log shows return within 500ms, `kill -CONT <pid>` restore
  - [ ] Rapid double-click debounce: 2 clicks at 50ms interval → exactly one `minimize dispatched` log line
  - [ ] Click during launch animation: `open -a <target>`, immediate Dock click within 100ms → no crash, no assertion
  - [ ] Multi-window: target with 2 open windows, click Dock while frontmost → only focused window minimizes, `window-count` drops by exactly 1
  - [ ] Right-click + ctrl-click on Dock item → no `minimize dispatched` log entry (deliberate scope)
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] Baseline: record current Dock settings (orientation, tilesize, autohide) so the script restores on exit (trap EXIT)
  - [ ] Resize: `defaults write com.apple.dock tilesize -int 96 && killall Dock` → log shows frame refresh within 2s
  - [ ] Move: orientation left → right → bottom → log shows refresh for each
  - [ ] Auto-hide on → one click on the configured edge strip minimizes target; auto-hide off → frame re-cached to full Dock rect
  - [ ] Cleanup: restores baseline on exit even if a sub-case fails
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] Time Profiler, 1000 clicks **outside** Dock: `AXUIElementCopyElementAtPosition` does not appear in the top-10 heaviest stacks
  - [ ] Time Profiler, 1000 clicks **inside** Dock: `AXUIElementCopyElementAtPosition` appears but accounts for <30% of sample time
  - [ ] Allocations, 5min idle: resident memory mean <10 MB, peak <15 MB
  - [ ] Allocations, 500 Dock clicks: net heap growth <200 KB (i.e., no per-click leak)
  - [ ] Emits structured stdout ready to paste into `PERF.md`
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] TCC first-grant flow: fresh checkout, unsigned quarantine cleared, launch → prompt fires → grant → menu bar icon active, clicks intercepted
  - [ ] Sleep/wake re-check: `pmset sleepnow`, wake, confirm monitor still running (log stream shows re-check)
  - [ ] Display hot-plug: unplug external monitor (if any) → click Dock on remaining screen → minimize still works → replug → log shows frame refresh
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] `./qa/run-all.sh` runs all CI-safe + AX-dependent suites in sequence
  - [ ] `./qa/run-all.sh --ci` runs only the CI-safe subset (suite 01) and exits cleanly on a non-grantable runner
  - [ ] `./qa/run-all.sh --baseline` writes both the Baseline and Current columns of `PERF.md`
  - [ ] Report file is a clean markdown table, one row per suite, with links to per-suite logs in `qa/reports/<timestamp>/`
  - [ ] Regression guard: if any `PERF.md` Current value regresses >20% vs. Baseline, exit code 1 and the report header is flagged `REGRESSION`
  - [ ] README gets a new "Running the QA suite" section pointing at `qa/run-all.sh` and listing prereqs
- **Verification step**:
...
- [ ] `cliclick` not documented as a prereq — suite fails with cryptic "command not found" instead of a clear message
- [ ] AXProbe binary not granted Accessibility — every suite fails identically; add a preflight check in `common.sh` that runs `AXIsProcessTrusted()` via AXProbe and prints the grant path if missing
- [ ] Dock config suite does not restore state on failure — leaves maintainer's Dock mangled; verify the `trap EXIT` restore path by intentionally failing a sub-case
- [ ] `xctrace` parser hard-codes column indices — Xcode upgrade silently breaks perf numbers; enforce column-name assertion
- [ ] Perf regression >20% merged without release-notes mention — `run-all.sh --baseline` flag must be explicit, not default, so accidental re-baselining can't hide a regression
- [ ] CI-safe suite confused with full suite — clearly label in output which subset ran
