...
- **Acceptance criteria**:
  - [x] Runner pinned to specific tag (comment rationale)
  - [x] Xcode selected via `sudo xcode-select -s /Applications/Xcode_XX.Y.app` (match the pinned runner's available versions)
  - [x] **Dated pin comment immediately above the `xcode-select` step**, of the form `# pinned YYYY-MM-DD, Xcode versions on this runner: <list>` — re-dated every time the runner image or Xcode pin is bumped
  - [x] Comment links to https://github.com/actions/runner-images for the source of truth on available Xcode versions per runner
  - [x] `build-test` runs `swift build -c debug` then `swift test --parallel`
  - [x] Test results uploaded as artifact on failure
  - [x] `bundle-check` depends on `build-test`, runs `./build.sh`, `codesign --verify --verbose`, `plutil -lint`, uploads `.app` artifact (7-day retention)
  - [x] `lint` parallel to `build-test`, runs `swiftformat --lint Sources Tests`
  - [x] Concurrency group cancels stale PR runs
  - [x] `.build/` cache step
- **Verification step**:
...
- **Acceptance criteria**:
  - [x] Trigger filter: `tags: ['v*.*.*']`
  - [x] Reuses `build.sh`
  - [x] Zips `ClickToMin.app` preserving symlinks (`ditto -c -k --keepParent`)
  - [x] Creates GitHub Release via `softprops/action-gh-release` or equivalent, with zip attached
  - [x] Workflow notes in comment: "add notarization before distributing beyond personal use"
- **Verification step**:
...
- **Acceptance criteria**:
  - [x] `.swiftformat` exists and is respected by `swiftformat --lint`
  - [x] Config is minimal (a handful of rules; no project-wide rewrite)
  - [x] README or PR template references `swiftformat` as local hook
- **Verification step**:
...
- **Acceptance criteria**:
  - [x] Require PR before merging: ON
  - [x] Required approvals: 0 (solo) — bump to 1+ when collaborators join
  - [x] Dismiss stale approvals: ON
  - [x] Required status checks: `build-test`, `bundle-check`, `lint`
  - [x] Require branches up to date: ON
  - [x] Require conversation resolution: ON
  - [x] Require linear history: ON
  - [x] Do not allow bypassing: ON (applies to admins)
  - [x] Direct push list: empty
  - [x] Force pushes: OFF
  - [x] Deletions: OFF
  - [x] Repo-level merge settings: squash only (merge + rebase disabled); default commit = PR title + description
- **Verification step**:
...
- [x] Runner pinned to `macos-latest` — toolchain drift silently fails future builds
- [x] Xcode version not explicitly selected — runner default shifts, tests break unpredictably
- [x] Zip uses `zip -r` instead of `ditto` — bundle symlinks corrupted, launch fails
- [x] Release trigger pattern matches pre-release tags unintentionally (`v*` vs `v*.*.*`)
- [x] `lint` job fails but doesn't block merge — check name not added to required list
- [x] Branch protection configured but "Do not allow bypassing" left OFF — solo admin footgun
- [x] Concurrency group missing → PR CI queues stale runs, burns minutes
