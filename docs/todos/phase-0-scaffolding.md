...
- **Acceptance criteria**:
  - [ ] `Package.swift` declares `platforms: [.macOS(.v13)]`
  - [ ] `ClickToMinCore` target compiles from `Sources/ClickToMin/Core` only
  - [ ] Executable target depends on `ClickToMinCore` and the I/O sources
  - [ ] `ClickToMinTests` target depends on `ClickToMinCore` (not the executable)
  - [ ] `swift build` succeeds on an empty source tree with placeholder files
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] Every file in PLAN.md §File Structure exists
  - [ ] Every Swift stub compiles (empty `enum Placeholder {}` or similar)
  - [ ] `swift build` succeeds
  - [ ] `swift test` runs zero tests and exits 0
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] `PERF.md` exists with all four sections from PLAN.md §File Structure note (steady-state metrics, short-circuit effectiveness, OS observations, release history)
  - [ ] Each table has its full header row and at least one `TBD` row
  - [ ] `.swiftformat` exists and parses (`swiftformat --lint .` exits without a config error)
- **Verification step**:
...
- **Acceptance criteria**:
  - [ ] File exists at the canonical GitHub path
  - [ ] Contains all four checklist items from PLAN.md §PR Hygiene
- **Verification step**:
...
- [ ] Core target accidentally imports AppKit (try it, must fail)
- [ ] Test target accidentally depends on executable (circular; must not)
- [ ] Placeholder files left with non-compiling content
- [ ] Missing `Resources/Info.plist` directory — Phase 4 will silently break
